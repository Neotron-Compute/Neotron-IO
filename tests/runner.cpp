...
DEFINE_TEST( hid_encode_main_descriptor )
{
	uint8_t buffer[64] = { 0 };
	HidDescriptor d = HidDescriptor(
	    /* wReportDescLength */ 3,
	    /* wReportDescRegister */ 4,
	    /* wInputRegister */ 5,
	    /* wMaxInputLength */ 6,
	    /* wOutputRegister */ 7,
	    /* wMaxOutputLength */ 8,
	    ...
	    /* wProductID */ 12,
	    /* wVersionID */ 13 );
	const uint8_t example[30] = {
	    30, 0, 0, 1,    3,    0,  4, 0,  5, 0,  6, 0, 7, 0, 8,
	    0,  9, 0, 0xFE, 0xFF, 11, 0, 12, 0, 13, 0, 0, 0, 0, 0,
	};

	size_t result = d.encode_into_buffer( buffer, sizeof( buffer ) );
	bool pass = ( result == 30 ) &&
	            ( memcmp( buffer, example, sizeof( example ) ) == 0 );
	if ( !pass )
	{
		for ( size_t i = 0; i < sizeof( example ); i++ )
		{
			printf( "%02x = %02x? %c\r\n", buffer[i], example[i],
			        buffer[i] == example[i] ? 'Y' : 'N' );
		}
		printf( "result = %zu\r\n", result );
	}
	return pass;
...
DEFINE_TEST( hid_encode_command )
{
	uint8_t buffer[4] = { 0 };
	HidCommand d = HidCommand( HidCommandOpcode::OPCODE_GET_REPORT,
	                           HidCommandReportType::REPORT_TYPE_INPUT, 6 );
	const uint8_t example[2] = { 2, ( 1 << 4 ) + 6 };
	size_t result = d.encode_into_buffer( buffer, sizeof( buffer ) );
	bool pass = ( result == 2 ) &&
	            ( memcmp( buffer, example, sizeof( example ) ) == 0 );
	if ( !pass )
...
const test_fn_t TESTS[] = {
    ps2_timeout,      ps2_collect_bits,           ps2_validate_words,
    ps2_encode_bytes, hid_encode_main_descriptor, hid_encode_command,
};
