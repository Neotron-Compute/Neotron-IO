 ...
 * \return value read
 */
static inline __attribute__( ( always_inline ) ) bool fastDigitalRead(
    uint8_t pin )
{
	badPinCheck( pin );
...
class DigitalPin
{
   public:
	 ...
	 * \param[in] mode If true set output mode else input mode
	 * \param[in] level If mode is output, set level high/low.
	 *                  If mode is input, enable or disable the pin's 20K
	 * pullup.
	 */
	inline __attribute__( ( always_inline ) ) void config( bool mode,
	                                                       bool level )
	{
	...
	//----------------------------------------------------------------------------
	/** \return Pin's level */
	inline __attribute__( ( always_inline ) ) bool read()
	{
